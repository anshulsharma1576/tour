    ...
    if(isset($_POST['btn']))
     {
        ...
        $sql="Select * from flog where usermail='".$lmail."' and userpassword='".$lpassword."' "; 
        $qs= mysqli_query($con, $sql);
        $row=mysqli_num_rows($qs);
        // print_r($row);
        // die;
        if($row>=1){
            session_start();
            ...
            ?>
            <script>
            alert ("You are not registerd with us kindly signup");
            </script>
        <?php
...
<form action="" method="POST">
                        <div class="form-floating mb-3">
                          <label for="floatingInput">Email address</label>
                            <input type="text" name="lmail" class="form-control"  placeholder="userid">
                        </div>
                        <div class="form-floating mb-4">
                        <label for="floatingPassword">Password</label>
                            <input type="password" name="lpassword" class="form-control" id="floatingPassword" placeholder="Password">
                        </div>  
                        <div class=row>
                            <div class=col-md-6>
